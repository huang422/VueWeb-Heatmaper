...
6. 新增平日、假日切換按鈕功能，day_type欄位內容分別分為“假日” “平日”
7. 不要調整日期或時間地圖就重新縮放reload要讓切換順暢顯示數值
8. 同時全面檢查更新相關文件規格和程式碼

1. 先詳細閱讀所有程式碼和文件，將標題改成Data Visualization HeatMap
2. 後端啟動失敗請全面檢查
3. 檢查所有功能與顯示都要正常運作
4. 全面檢查所有程式碼與文件

1. 將預設的地圖畫面縮放放到點呈現的畫面，點用清楚的顏色標示出來，確認經緯
度計算結果呈現在地圖的正確位置

全面檢查所有程式碼與文件移除冗餘的程式碼或修改錯誤，測試程式碼可以不需要
1. 固定地圖位置不然輪播的時候會重啟地圖，一開始就載入固定的地圖位置就好，輪播的時候只要看數值變化就好地圖不能跟著變化
2. 熱力圖的點用當下時間點的15個點呈現熱力圖強度範圍，將點加大，加大範圍約外擃地圖上距離25公尺

1. 在readme後方demo 圖片，圖片路徑data/images/img_1
2. 在readme文件要提到因為資料隱私不能上傳github
3. data取消上傳git, prompt.md也不用git